...
ForEach ($ymlFile in $ymlFiles ) {
    ...
    foreach ($service in $buildYaml.services.values) {
        ...
        if($dockerRepository -eq 'xc-resources')
        {
            # If this is the xc-resources image, dont tag it below as the xc-resources image is throw away
            # and only used as an initial base image for the other xc images.  
            # By not tagging, this will leave the image as latest and therefore it will not be pushed to the docker repo
            # in the container push task
            Write-Host "Skipping tagging of Docker Repository: $dockerRepository" -InformationAction Continue
            continue;
        }
