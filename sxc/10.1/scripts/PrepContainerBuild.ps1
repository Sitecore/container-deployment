...
Path to the Sitecore XC release zip files downloaded from dev.sitecore.net.
...
if (Test-Path("$jsonFile"))
{
    ...
    if ($releaseFound -lt 1) {
        Throw 'No Sitecore XC release zip files were found.'
    }
            ...
            if (Test-Path $wdpFile.FullName) {
                ...
                if($j | Get-Member "ToCopyFiles"){
                    ...
                    foreach ($file in $files) {
                        ...
                        if($null -ne $nameMapping -and $nameMapping.NewName -ne ""){
