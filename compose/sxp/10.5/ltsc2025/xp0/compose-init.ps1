...
Param (
    ...
    [string]
    $SharedCertificateFolder = ".\shared",
    ...
    [string]
    $SolrConnectionUserName = "sitecore",

    [Parameter(Mandatory = $true)]
    [string]
    $SolrConnectionPassword,
...
function Create-Certificates{
    param(
        ...
        [string]$IdHost,
        [string]$IdFolder,
        [string]$SharedCertificateFolder,
        [string]$MsSql,
        [string]$MsSqlFolder,
	...
	if (-not $existingCertificateFiles){
        ...
        # Copy RootCA.crt into shared\certs for non-id roles.
        $sharedCertificatesPath = Join-Path $SharedCertificateFolder "certs"
        New-Item -ItemType Directory -Path $sharedCertificatesPath -Force | Out-Null
        Copy-Item "$CertDataFolder\RootCA.crt" -Destination $sharedCertificatesPath -Force
...
function Invoke-ComposeInit {
    ...
    $envVariablesTable = @{ 
        ...
        "SOLR_ADMIN_USERNAME" = $SolrAdminUserName
        "SOLR_ADMIN_PASSWORD" = $SolrAdminPassword
        "SOLR_CONNECTION_USERNAME" = $SolrConnectionUserName
        "SOLR_CONNECTION_PASSWORD" = $SolrConnectionPassword
    ...
    if($envFile -eq "upgrade.env"){
        ...
        $RootCertificateCreated = Create-Certificates -CertDataFolder $CertDataFolder -Topology $Topology -CdHost $CdHost -CmHost $CmHost -IdHost $IdHost -IdFolder $IdFolder -SharedCertificateFolder $SharedCertificateFolder -MsSql $SqlServer -MsSqlFolder $SqlServerFolder -MsSqlCertPswd $SqlServerCertificatePassword
